...
class Contact extends Component {
  ...
  validate(firstname, lastname, telnum, email) {
    ...
    if (this.state.touched.lastname && lastname.length < 3) {
      ...
      errors.lastname = "Last name must be shorter than 15 characters";
    }

    const reg = /^((\+){1}91){1}[1-9]{1}[0-9]{9}$/g;
    if (this.state.touched.telnum && !reg.test(telnum)) {
      errors.telnum = "Contact must be a valid number";
    }
    const regMail = /^([a-zA-Z0-9_]+)@([a-zA-Z0-9_]+)\.([a-zA-Z]{2,5})$/g;
    if (this.state.touched.email && !regMail.test(email)) {
